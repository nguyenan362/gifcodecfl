...
remove_cloudflare_tunnel() {
  ...
  if systemctl list-unit-files "${CFL_CLOUDFLARED_SERVICE_NAME}.service" >/dev/null 2>&1; then
    systemctl disable "${CFL_CLOUDFLARED_SERVICE_NAME}" >/dev/null 2>&1 || true
    systemctl stop "${CFL_CLOUDFLARED_SERVICE_NAME}" >/dev/null 2>&1 || true
    rm -f "${CFL_SYSTEMD_DIR}/${CFL_CLOUDFLARED_SERVICE_NAME}.service"
